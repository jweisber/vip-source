...
But here's the thing: these choices contradict the rule of maximizing expected utility! It's not obvious at first. But a few lines of algebra will prove that it's so.
...
`r newthought("Visually")`, the St. Petersburg game looks like this:

```{r echo=FALSE, fig.cap="The St. Petersburg game"}
...
`r newthought("Most people")`, when they first encounter this puzzle, try to take the easy way out. "The game is impossible", they say. "Nobody could actually fund this game in real life. No casino (or government) has an unlimited amount of money. Besides, the coin would wear out after a while, so the house couldn't truly guarantee they'd be able to come through."

There are two serious problems with this solution, however. 
...
For example, on Bernoulli's scale winning $\$64$ is only about *twice* as good as winning $\$8$, not eight times as good:
$$
  \begin{aligned}
    \log(64) &\approx 4.09,\\
    \log(8)  &\approx 2.08.
