...
```{r echo=FALSE, cache=TRUE, fig.margin=TRUE, fig.cap="Mutually exclusive propositions don't overlap"}
...
```{r echo=FALSE, cache=TRUE, fig.margin=TRUE, fig.cap="Independent propositions do overlap (unless one of them has zero probability)."}
propositions <- data.frame(
    cirx = c(-.5   , .5),
    ciry = c(0     , 0),
    r    = c(1     , 1),
...
```{r echo=FALSE, cache=TRUE, fig.margin=TRUE, fig.cap="The Equivalence Rule. The $A \\vee B$ region is identical to the $B \\vee A$ region, so they have the same probability."}
propositions <- data.frame(
    cirx = c(-.75   , .75),
    ciry = c(0      , 0),
    r    = c(1.5    , 1.5),
