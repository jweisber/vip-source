...
`r newthought("Suppose")` your friend is one of these people. How might they respond to the calculation above? One thing they could do is replace the dollar amounts with different numbers, numbers that more accurately capture their personal values.
...
`r newthought("People")` have different desires, needs, and interests. Some value friendship most, others value family more. Some prioritize professional success, others favour activism. Some people like '80s music, others prefer hip-hop---and still others, to my constant surprise, enjoy reggae.
...
```{r echo=FALSE, fig.margin=TRUE, fig.cap="Frank Ramsey ($1903$--$1930$) died at the age of $26$, before his discovery could become widely known. Luckily the idea was rediscovered by economists and statisticians in the $1940$s."}
...
```{r echo=FALSE, fig.margin=TRUE, fig.cap="A utility scale for lunch options"}
text <- bind_rows(
...
ggplot(text) +
  ...
  geom_text(aes(x = x, y = y, label = label, hjust = hjust), size = 5)
```
...
```{r echo=FALSE, fig.margin=TRUE, fig.cap="A friend's utility scale for lunch options"}
text <- bind_rows(
  list(x = .6, y = 1,   label = "Pizza",  hjust = "left"),
...
`r newthought("Lunch")` is important, but how do we apply this recipe more broadly? How do we quantify the value a person places on other things, like career options, their love life, or their preferred music?
...
```{r uscale, echo=FALSE, fig.margin=TRUE, fig.cap="A utility scale with some arbitrarily selected points of comparison chosen for display: $u = 7/8$, $2/3$, $1/2$, and $1/6$"}
text <- bind_rows(
...
ggplot(text) +
  ...
  geom_text(aes(x = x, y = y, label = label, hjust = hjust), size = 5, parse = TRUE) +
...
`r newthought("This method")` for quantifying utility assumes that $\u(W) = 0$ and $\u(B) = 1$. What justifies this assumption? It's actually a bit arbitrary. We could measure someone's utilities on a scale from $0$ to $10$ instead. Or $-1$ to $1$, or any interval.
...
```{r echo=FALSE, fig.margin=TRUE, fig.show="hold", fig.cap="The best possible gamble (top), the worst possible gamble (middle), and an intermediate gamble (bottom)"}
ggplot(NULL, aes(c(0, 1))) + 
  ...
  ggtitle(expression(Best~Possible~Gamble:~italic(Pr)(italic(B))==1))
...
ggplot(NULL, aes(c(0, 1))) + 
  ...
  scale_y_continuous("utility", breaks = seq(0, 1, 1), limits = c(0, 1)) +
  scale_x_continuous("probability", breaks = seq(0, 1, 1)) +
  ggtitle(expression(Worst~Possible~Gamble:~italic(Pr)(italic(W))==1))

ggplot(NULL, aes(c(0, 1))) + 
  coord_fixed() +
  stat_function(fun = function(x) case_when(x <= 1/2 ~ 1/3, x <= 1 ~ 2/3), 
                geom = "area", n = 1000, fill = rightcolour) +
  scale_y_continuous("utility", breaks = seq(0, 1, 1), limits = c(0, 1)) +
  scale_x_continuous("probability", breaks = seq(0, 1, 1)) +
  ggtitle("A Gamble of Intermediate Value")
```
...
`r newthought("If you")` saw $50$ cents on the ground while walking, would you stop to pick it up? I probably wouldn't. But I would have when I was younger, back then I had a lot less money. So an additional $50$ cents made a bigger difference to my day. The more money you have, the less an additional $50$ cents will be worth to you.
