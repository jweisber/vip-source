...
What will happen if our researchers set the cutoff for statistical significance at $.05$? Only a few of the bogus treatments will be approved. Just $5\%$ of those studies will get flukey, statistically significant results. And half of those will look like they're harming patients' chances of recovery, rather than helpful. So only $2.5\%$ of the $160$ bogus treatments will be approved, which is $4$ treatments.
...
ggplot(NULL) +
  geom_path(aes(x = x, y = y), data = df_useless, colour = leftcolour) +
  geom_path(aes(x = x, y = y), data = df_effective, colour = green) +
...
ggplot() + 
  ...
  geom_path(aes(x = x, y = y), data = path, colour = green, size = 1, linetype = "longdash")
```
...
```{r lindley, echo=FALSE, cache=TRUE, fig.cap="An illustration of Lindley's paradox. The finding $k = 36$ doesn't fit well with either hypothesis, so both are rejected. But it fits one of them (red) much better than the other (blue). So the blue hypothesis is more plausible from an intuitive perspective."}
ggplot(data = NULL, aes(x = c(0, 40))) +
  stat_function(fun = dnorm, args = list(mean = 12, sd = 3), n = 201,
                colour = rightcolour) +
  stat_function(fun = dnorm, args = list(mean = 24, sd = 3.464), n = 201,
...
#. Suppose we have $1,000$ coins and we are going to conduct an experiment on each one to determine which are biased. Each coin will be tossed $100$ times. In each experiment, the null hypothesis is always that the coin is fair, and we will reject this hypothesis when the results of the experiment are significant at the $.05$ level.
    ...
    b. How many unfair coins should we expect will correctly end up abeled "unfair".
    d. What percentage of the coins labeled "unfair" should we expect to actually be unfair? 

#. Suppose we are going to investigate $1,000$ null hypotheses by running an experiment on each. In each experiment, we will reject the null hypothesis when the results are significance at the $.01$ level.
