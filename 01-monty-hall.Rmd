...
If so, you're in good company. Most people find this answer sensible, including some professors of statistics and mathematics. They figure there are only two possibilities remaining, door A and door B, each has the same one-in-two chance of being the winner. So it doesn't matter which one you pick.
...
```{marginfigure}
![](img/marg_fig.png)
The hundred-door version of the Monty Hall problem, suggested by Marilyn vos Savant
```

Here's another way to think about it. Imagine the game had a hundred doors instead of just three. And suppose again you start by picking the first door at random. Then the host opens *all the other doors but one*, door $59$ let's imagine. You have to ask yourself: why did they pick door $59$ to leave closed?? Almost certainly because that's where the prize is hidden! Maybe you got really lucky and picked right with the first door at the beginning. But it's way more likely you didn't, and the host had to keep door $59$ closed to avoid giving away the game.


## Diagramming the Solution

`r tufte::newthought("A picture helps")` clarify things. At first the prize could be behind any of the three doors, with equal probability each way. So we draw a tree with three branches, each labeled with a probability of $1/3$.

```{r echo=FALSE, cache=TRUE, fig.cap="First stage of a tree diagram for the Monty Hall problem"}
df_nodes <- rbind(
  data.frame(text = "C", x = .5, y = .9),
  data.frame(text = "B", x = .5, y = .5),  
  data.frame(text = "A", x = .5, y = .1),
  data.frame(text = "Open B", x = 1,  y = .9),  
  data.frame(text = "Open C", x = 1,  y = .5),
  data.frame(text = "Open B", x = 1,  y = .2),
  data.frame(text = "Open C", x = 1,  y = 0)
)

df_segments <- rbind(
  data.frame(x1 = 0, y1 = .5, x2 = .48, y2 = .9),
  data.frame(x1 = 0, y1 = .5, x2 = .48, y2 = .5),  
  data.frame(x1 = 0, y1 = .5, x2 = .48, y2 = .1),
  data.frame(x1 = .52, y1 = .9, x2 = .94, y2 = .9),  
  data.frame(x1 = .52, y1 = .5, x2 = .94, y2 = .5),
  data.frame(x1 = .52, y1 = .1, x2 = .94, y2 = .2),
  data.frame(x1 = .52, y1 = .1, x2 = .94, y2 = 0)
)

df_segment_labels <- rbind(
  data.frame(text = "1/3", x = .25, y = .75),
  data.frame(text = "1/3", x = .25, y = .53),  
  data.frame(text = "1/3", x = .25, y = .33),
  data.frame(text = "1/2", x = .75, y = .18),
  data.frame(text = "1/2", x = .75, y = .07)
)

df_leaf_labels <- rbind(
  data.frame(text = "1/3", x = 1.1, y = .9),
  data.frame(text = "1/3", x = 1.1, y = .5),
  data.frame(text = "1/6", x = 1.1, y = .2),
  data.frame(text = "1/6", x = 1.1, y = 0)
)

ggplot() +
  geom_text(aes(x, y, label = text), data = df_nodes[0:3, ], fontface = "italic") + 
  geom_segment(aes(x = x1, y = y1, xend = x2, yend = y2), data = df_segments[0:3, ]) + 
  geom_text(aes(x, y, label = text), data = df_segment_labels[0:3, ]) + 
  xlim(0, 1.1) + ylim(0, 1) +
  theme_void()
```
...
Only if the prize is behind door A do they have a choice: open either door B or door C. In that case it's a tossup which door they'll open, so each of those possibilities has a 1/2 chance.

```{r echo=FALSE, cache=TRUE, fig.cap="Second stage of a tree diagram for the Monty Hall problem"}
ggplot() +
...
When things follow the bottom branches, half of those times the host will open door B, and half the time they'll open door C. So one in every six plays will follow the *A-and-Open-B* path. And one in every six plays will follow the *A-and-Open-C* path.

```{r echo=FALSE, cache=TRUE, fig.cap="Complete tree diagram for the Monty Hall problem"}
ggplot() +
  geom_text(aes(x, y, label = text), data = df_nodes, fontface = "italic") + 
  geom_segment(aes(x = x1, y = y1, xend = x2, yend = y2), data = df_segments) + 
  geom_text(aes(x, y, label = text), data = df_segment_labels) + 
...
```{r echo=FALSE, warning=FALSE, cache=TRUE, fig.margin=TRUE, fig.height=10, fig.cap="Three  partitions for a card drawn from a standard deck"}
...
`r tufte::newthought("Tree diagrams")` follow a few simple rules based on these concepts. The parts of a tree are called *nodes*, *branches*, and *leaves*:

```{r echo=FALSE, cache=TRUE, fig.cap="Nodes, branches, and leaves"}
ggplot() +
  geom_text(aes(x, y, label = text), data = df_nodes, fontface = "italic") + 
  geom_segment(aes(x = x1, y = y1, xend = x2, yend = y2), data = df_segments) + 
  xlim(-.1, 1.1) + ylim(0, 1) +
  theme_void() +
  annotate(geom = "text", label = "Nodes", x = .1, y = .1, color = "firebrick") +
  annotate(geom = "segment", x = .1, y = .13, xend = 0.01, yend = .485, color = "firebrick", arrow = arrow(length = unit(0.01, "npc"), type = "closed")) +
  annotate(geom = "segment", x = .16, y = .1, xend = 0.48, yend = .09, color = "firebrick", arrow = arrow(length = unit(0.01, "npc"), type = "closed")) +
  annotate(geom = "text", label = "Branches", x = .6, y = .7, color = "steelblue") +
  annotate(geom = "segment", x = .52, y = .7, xend = 0.25, yend = .7, color = "steelblue", arrow = arrow(length = unit(0.01, "npc"), type = "closed")) +
  annotate(geom = "segment", x = .6, y = .73, xend = 0.65, yend = .89, color = "steelblue", arrow = arrow(length = unit(0.01, "npc"), type = "closed")) +
  annotate(geom = "segment", x = .6, y = .67, xend = 0.65, yend = .51, color = "steelblue", arrow = arrow(length = unit(0.01, "npc"), type = "closed")) +
  annotate(geom = "text", label = "Leaves", x = 1.1, y = .35, color = "forestgreen") +
  annotate(geom = "segment", x = 1.1, y = .32, xend = 1, yend = .23, color = "forestgreen", arrow = arrow(length = unit(0.01, "npc"), type = "closed")) +
  annotate(geom = "segment", x = 1.1, y = .37, xend = 1, yend = .47, color = "forestgreen", arrow = arrow(length = unit(0.01, "npc"), type = "closed"))
```
