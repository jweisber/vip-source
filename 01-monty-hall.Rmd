...
```{r fig.cap="First stage of a tree diagram for the Monty Hall problem"}
df_nodes <- rbind(
...
```{r fig.cap="Second stage of a tree diagram for the Monty Hall problem"}
ggplot() +
...
```{r fig.cap="Complete tree diagram for the Monty Hall problem"}
ggplot() +
  geom_text(aes(x, y, label = text), data = df_nodes, fontface = "italic") + 
  geom_segment(aes(x = x1, y = y1, xend = x2, yend = y2), data = df_segments) + 
  geom_text(aes(x, y, label = text), data = df_segment_labels) + 
...
```{r warning=FALSE, fig.margin=TRUE, fig.height=10, fig.cap="Three  partitions for a card drawn from a standard deck"}
...
```{r fig.cap="Nodes, branches, and leaves"}
ggplot() +
  geom_text(aes(x, y, label = text), data = df_nodes, fontface = "italic") + 
  geom_segment(aes(x = x1, y = y1, xend = x2, yend = y2), data = df_segments) + 
  xlim(-.1, 1.1) + ylim(0, 1) +
