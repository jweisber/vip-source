...
```{marginfigure}
We can make the game instantaneous too. Instead of flipping a coin over and over, we throw a dart once at a square board. If it lands in the left half, that corresponds to heads on the first toss. If it lands in the bottom-right quadrant, that corresponds to heads on the second toss. If it lands in the top-right quadrant, we divide it into halves and quadrants similarly, and so on.
```
