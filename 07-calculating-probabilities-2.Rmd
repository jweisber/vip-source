...
We can also use a tree diagram to illustrate the same reasoning. There are two leaves where $A$ is true, marked with emoji. To get the probability of each leaf we multiply across the branches (that's the General Multiplication Rule). And then to get the total probability for $A$, we add up the two leaves: $\p(A) =$ `r png_shades` $+$ `r png_hearts`. Once again the result is the Law of Total Probability:
$$
  \p(A) = \p(A \given B) \p(B) + \p(A \given \neg B) \p(\neg B).
$$
