...
ggplot() + theme_void() + coord_fixed() +
  ...
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1),
        panel.background = element_rect(fill = leftcolour)) +
...
ggplot() + 
  ...
  xlim(-3,3) + ylim(-2,2) +
  geom_circle(aes(x0 = -.75, y0 = 0, r = 1.5), fill = leftcolour) +  
  geom_circle(aes(x0 = .75, y0 = 0, r = 1.5), fill = rightcolour) +  
  geom_ribbon(aes(x = x, ymin = upper(x), ymax = -upper(x)), 
              fill = middlecolour, colour = "black") +
...
nodes <- rbind(
  data.frame(text = "A",   x = .39, y = 5/6),
  data.frame(text = "~ A", x = .39, y = 1/6),  
  data.frame(text = "B",   x = .78, y = 1  ),  
  data.frame(text = "~ B", x = .78, y = 2/3),
  data.frame(text = "B",   x = .78, y = 1/3),
  data.frame(text = "~ B", x = .78, y = 0  )
)

segments <- rbind(
  data.frame(x1 = 0,   y1 = .5,  x2 = .36, y2 = 5/6 ),
  data.frame(x1 = 0,   y1 = .5,  x2 = .36, y2 = 1/6 ),  
  data.frame(x1 = .42, y1 = 5/6, x2 = .75, y2 = 1   ),  
  data.frame(x1 = .42, y1 = 5/6, x2 = .75, y2 = 2/3 ),
  data.frame(x1 = .42, y1 = 1/6, x2 = .75, y2 = 1/3 ),
  data.frame(x1 = .42, y1 = 1/6, x2 = .75, y2 = 0   )
)

leaf_labels <- rbind(
  data.frame(text = "Pr(A&B)",   x = .9, y = 1),
  data.frame(text = "Pr(A&~B)",  x = .9, y = 2/3),
  data.frame(text = "Pr(~A&B)",  x = .9, y = 1/3),
  data.frame(text = "Pr(~A&~B)", x = .9, y = 0)
)

shades <- readPNG("img/emoji_shades.png") %>% rasterGrob()
hearts <- readPNG("img/emoji_hearteyes.png") %>% rasterGrob()
...
nodes <- rbind(
  data.frame(text = "H", x = 1/2, y = 5/6),
  data.frame(text = "T", x = 1/2, y = 1/6),  
  data.frame(text = "B", x = 1,   y = 1  ),  
  data.frame(text = "W", x = 1,   y = 2/3),
  data.frame(text = "B", x = 1,   y = 1/3),
  data.frame(text = "W", x = 1,   y = 0  )
)

segments <- rbind(
  data.frame(x1 = 0,   y1 = .5,  x2 = .48, y2 = 5/6),
  data.frame(x1 = 0,   y1 = .5,  x2 = .48, y2 = 1/6),  
  data.frame(x1 = .52, y1 = 5/6, x2 = .98, y2 = 1  ),  
  data.frame(x1 = .52, y1 = 5/6, x2 = .98, y2 = 2/3),
  data.frame(x1 = .52, y1 = 1/6, x2 = .98, y2 = 1/3),
  data.frame(x1 = .52, y1 = 1/6, x2 = .98, y2 = 0  )
)

segment_labels <- rbind(
  data.frame(text = "1/2", x = .25, y = .72),
  data.frame(text = "1/2", x = .25, y = .38),  
  data.frame(text = "3/4", x = .75, y = .95),
  data.frame(text = "1/4", x = .75, y = .78),
  data.frame(text = "1/4", x = .75, y = .29),
  data.frame(text = "3/4", x = .75, y = .13)
)

leaf_labels <- rbind(
  data.frame(text = "3/8", x = 1.1, y = 1  ),
  data.frame(text = "1/8", x = 1.1, y = 2/3),
  data.frame(text = "1/8", x = 1.1, y = 1/3),
  data.frame(text = "3/8", x = 1.1, y = 0  )
)
...
nodes <- rbind(
  data.frame(text = "X",   x = .5, y = 5/6),
  data.frame(text = "Y",   x = .5, y = 1/6),  
  data.frame(text = "BB",  x = 1,  y = 1  ),  
  data.frame(text = "~BB", x = 1,  y = 2/3),
  data.frame(text = "BB",  x = 1,  y = 1/3),
  data.frame(text = "~BB", x = 1,  y = 0  )
)

segments <- rbind(
  data.frame(x1 = 0,   y1 = .5,  x2 = .48, y2 = 5/6),
  data.frame(x1 = 0,   y1 = .5,  x2 = .48, y2 = 1/6),  
  data.frame(x1 = .52, y1 = 5/6, x2 = .96, y2 = 1  ),  
  data.frame(x1 = .52, y1 = 5/6, x2 = .96, y2 = 2/3),
  data.frame(x1 = .52, y1 = 1/6, x2 = .96, y2 = 1/3),
  data.frame(x1 = .52, y1 = 1/6, x2 = .96, y2 = 0  )
)

segment_labels <- rbind(
  data.frame(text = "1/2",   x = .25, y = .72),
  data.frame(text = "1/2",   x = .25, y = .38),  
  data.frame(text = "9/16",  x = .75, y = .95),
  data.frame(text = "7/16",  x = .75, y = .78),
  data.frame(text = "1/16",  x = .75, y = .29),
  data.frame(text = "15/16", x = .75, y = .12)
)

leaf_labels <- rbind(
  data.frame(text = "9/32", x = 1.1, y = 1  ),
  data.frame(text = "1/32", x = 1.1, y = 1/3)
)
...
We can also use an Euler diagram. The size of the $A$ region is the sum of the $A \wedge B$ region and the $A \wedge \neg B$ region: $\color{middlecolour}{\blacksquare}\color{black}{} + \color{leftcolour}{\blacksquare}\color{black}{}$. And each of those regions can be calculated using the General Multiplication Rule. For example, $\p(A \wedge B) = \p(A \given B) \p(B)$. So in algebraic terms we have:
$$
  \begin{aligned}
...
We can also use a tree diagram to illustrate the same reasoning. There are two leaves where $A$ is true, marked 😎 and 😍. To get the probability of each leaf we multiply across the branches (that's the General Multiplication Rule). And then to get the total probability for $A$, we add up the two leaves:
$$
  \begin{aligned}
    \p(A) &= 😎 + 😍\\
          &= \p(A \given B) \p(B) + \p(A \given \neg B) \p(\neg B).
...
nodes <- rbind(
  data.frame(text = "B",   x = .5, y = 5/6),
  data.frame(text = "D",   x = .5, y = 1/6),  
  data.frame(text = "O",   x = 1,  y = 1  ),  
  data.frame(text = "~ O", x = 1,  y = 2/3),
  data.frame(text = "O",   x = 1,  y = 1/3),
  data.frame(text = "~ O", x = 1,  y = 0  )
)
...
segment_labels <- rbind(
  data.frame(text = "1/4",   x = .25, y = .72),
  data.frame(text = "3/4",   x = .25, y = .38),  
  data.frame(text = "9/10",  x = .75, y = .95),
  data.frame(text = "1/10",  x = .75, y = .78),
  data.frame(text = "8/10",  x = .75, y = .29),
  data.frame(text = "2/10",  x = .75, y = .12)
)

leaf_labels <- rbind(
  data.frame(text = "9/40",  x = 1.1, y = 1  ),
  data.frame(text = "24/40", x = 1.1, y = 1/3)
)
