...
ggplot(df) +
  ...
  scale_fill_manual(values = c(rightcolour, leftcolour)) +
...
```{r echo=FALSE, fig.margin=TRUE, fig.cap="A North American roulette wheel"}
knitr::include_graphics("img/roulette_wheel.png")
```
...
```{r echo=FALSE, fig.margin=TRUE, fig.asp = .3, fig.cap="The continuous version of the Principle of Indifference: $Pr(H)$ is the length of the $c$-to-$d$ interval divided by the length of the whole $a$-to-$b$ interval."}
library(grid)
library(pBrackets)
ggplot(NULL) +
...
```{r echo=FALSE, cache=TRUE, fig.margin=TRUE, fig.cap="Joseph Bertrand ($1822$--$1900$) presented this paradox in his $1889$ book *Calcul des Probabilités*. He used a different example though. Our example is a bit easier to understand, and comes from the book *Laws & Symmetry* by Bas van Fraassen."}
