...
`r newthought("Probability")`  theory has three axioms, and they're all familiar laws of probability. But they're fundamental laws in a way. All the other laws can be derived from them.
...
`r newthought("Let's start")` with one of the easier laws to derive.
...
`r newthought("Our")`  next theorem is about conditional probability. But the concept of conditional probability isn't mentioned in the axioms, so we need to define it first.
...
`r newthought("Next")`  we'll prove the Equivalence rule and the General Addition rule. These proofs are longer and more difficult than the ones we've done so far.
...
`r newthought("Next")`  we derive the Law of Total Probability and Bayes' theorem.
...
`r newthought("Finally, let's introduce")` the concept of independence, and two key theorems that deal with it.
