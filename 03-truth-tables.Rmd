...
In this chapter we'll introduce the last few concepts we need from deductive logic, and we'll learn a helpful tool: truth tables.
...
Here we've used two simple propositions to build up longer, more complex ones using the terms *and*, *either/or*, and *it's not true that*. Such terms are called ***connectives***.^[Notice, we call *it's not true that* a connective even though it doesn't actually connect two propositions together.]
...
Here are some more examples using these symbols:

- $F \wedge \neg G$: Florida is warm and Geneva is not.
- $\neg J \vee \neg K$: Either Jing will not be at the party or Kamal won't be.

Sometimes we also need parentheses to avoid ambiguity. Consider the statement:
$$ 4 \div 2 \times 2 = 1. $$
Is it correct? That depends on what you mean. Does the division operation come first, or the multiplication? So we use parentheses to clarify: $4 \div (2 \times 2) = 1$, but $(4 \div 2) \times 2 = 4$.

Likewise in logic, we use parentheses to prevent ambiguity. Consider:
$$ A \vee B \wedge C. $$
This proposition is ambiguous. It has two interpretations, which in English we can distinguish with a comma:
...
The main connective is the last one used to build up the proposition. In $A \vee \neg B$ the main connective is the $\vee$.
