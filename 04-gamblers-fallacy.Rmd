...
```{r eikosex, echo=FALSE, fig.margin=TRUE, fig.cap="Example of an Eikosogram where $A$ and $B$ are *not* independent."}
ggplot() +
...
We can visualize independence with an ***eikosogram***, which is like an Euler diagram but with rectangles instead of circles. The size of each sector reflects its probability. For example, in Figure \@ref(fig:eikosex) the proposition $A \wedge B$ has low probability ($1/12$), while $\neg A \wedge B$ has much higher probability ($1/2$).
...
When propositions are independent, the eikosogram is divided by just two straight lines. That way the $A$ region takes up the same percentage of the $B$ region as it does the $\neg B$ region. So the probability of $A$ is the same whether $B$ is true or false. In other words, whether $B$ is true or not doesn't affect the probability of $A$.
